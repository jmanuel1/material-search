...
navigator.serviceWorker.ready.then(function () {
  ...
  QUnit.test('First declared route wins', function (assert) {
    // Matches both routes
    checkValue('multiple/match/anything.html', '1', assert); // Only matches second route
  ...
  QUnit.test('Caching', function (assert) {
    ...
    var date = Date.now();
    var url = 'cache/' + date;
    var done = assert.async(); // Confirm that the URL cannot be fetched

    var step1 = fetch(url); // If the fetch succeeds then we have a problem

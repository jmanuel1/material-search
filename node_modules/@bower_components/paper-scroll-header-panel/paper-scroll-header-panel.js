...
`,
  ...
  observers: ['_setup(headerHeight, condensedHeaderHeight, fixed)', '_condensedHeaderHeightChanged(condensedHeaderHeight)', '_headerHeightChanged(headerHeight, condensedHeaderHeight)', '_condensesChanged(condenses, headerHeight)'],
  listeners: {
  ...
  _condenseHeader: function (y) {
    var reset = y === null; // adjust top bar in paper-header so the top bar stays at the top
    ...
    this._translateY(this.$.headerBg, reset ? null : y / 2); // transition condensed header bg


    if (!this.noDissolve) {
      this.$.condensedHeaderBg.style.opacity = reset ? '' : y / this._headerMargin; // adjust condensed header bg so it stays at the center

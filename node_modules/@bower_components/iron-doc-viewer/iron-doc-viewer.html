...
<dom-module id="iron-doc-viewer">
  ...
  Polymer({
    ...
    ready: function () {
      var jsonDescriptor = this._loadJson(); // Note that this is only an error during element creation. You are free
    ...
    _descriptorChanged: function () {
      if (!this.descriptor) return; // Split the documented properties between functions and other types.
    ...
    _collapsedChanged: function () {
      this._collapseToggleLabel = this._collapsed ? 'expand' : 'collapse'; // Bound values aren't exposed to dom-repeat's scope.
    ...
    _noneToShow: function (showPrivate, items) {
      if (showPrivate === undefined || items === undefined) {
        return true;
      }

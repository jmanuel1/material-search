...
<script>(function () {
  'use strict'; // TODO: Doesn't work for IE or Safari, and the usual
  ...
  var VERSION = 1; // This allows us to use the PushSubscription attribute type in browsers
  ...
  Polymer({
    ...
    enable: function () {
      ...
      return navigator.serviceWorker.register(this._getWorkerURL(), {
        scope: SCOPE
    ...
    ready: function () {
      if (this.supported) {
        var handler = this._messageHandler.bind(this); // NOTE: We add the event listener twice because the specced and

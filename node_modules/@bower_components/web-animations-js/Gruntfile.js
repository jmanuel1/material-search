...
module.exports = function (grunt) {
  ...
  function genTarget(target) {
    ...
    var newGens = [generateFromTemplate('templates/web-animations.js', {
      target: target
    }, target + '.dev.js'), generateFromTemplate('templates/web-animations.html', {
      src: config.src
    }, target + '.dev.html'), generateFromTemplate('templates/runner.html', {
      target: target
    }, 'test/runner-' + target + '.html')];
    return newGens;
  }
  ...
  function build(target) {
    var config = targetConfig[target];
    var steps = [concat(config.scopeSrc.concat(config.sharedSrc), 'inter-' + target + '-preamble.js', concatDefines), concat(config.webAnimations1Src, 'inter-component-' + target + '-web-animations-1.js', concatDefines), guard('inter-component-' + target + '-web-animations-1.js', 'inter-guarded-' + target + '-web-animations-1.js'), concat(config.webAnimations1BonusSrc, 'inter-bonus-' + target + '.js', concatDefines)];
    var filenames = ['inter-' + target + '-preamble.js', 'inter-guarded-' + target + '-web-animations-1.js', 'inter-bonus-' + target + '.js'];
    ...
    steps.push(concatWithMaps(filenames, 'inter-' + target + '.js'), compress('inter-' + target + '.js', target + '.min.js', concatDefines));
  ...
  grunt.initConfig({
    ...
    gjslint: {
      options: {
        flags: ['--nojsdoc', '--strict', '--disable 7,121,110'],
        reporter: {
          name: 'console'
        }
      },
      all: {
        src: ['src/*.js', 'test/*.js', 'test/js/*.js']
      }
    },
    test: testTargets,
    debug: testTargets,
  ...
  function runTests(task, configCallback, testFilter) {
    ...
    function runWebPlatformTests(withNativeFallback) {
      ...
      return runKarma(function (karmaConfig) {
        ...
        var servedFiles = ['test/web-platform-tests/resources/**', 'test/web-platform-tests/web-animations/**', 'test/resources/*', 'src/**', '*.js*', // TODO(alancutter): Make a separate grunt task for serving these imported Blink tests.
        'test/blink/**'];
  ...
  grunt.task.registerMultiTask('sauce', 'Run <target> tests under Karma on Saucelabs', function () {
    ...
    runTests(this, function (karmaConfig) {
      ...
      karmaConfig.sauceLabs = {
        testName: 'web-animation-next ' + target + ' Unit tests'
      };
    });
  });
  grunt.task.registerMultiTask('sourceMapConcat', 'concat source files and produce combined source map', function () {
    ...
    var out = "";
    var outMapGenerator = new sourceMap.SourceMapGenerator({
      file: this.target
    });
    var lineDelta = 0;
  ...
  grunt.task.registerMultiTask('wrap', 'Wrap <target> source file and update source map', function () {
    var inFile = grunt.file.read(this.data.source);
    var inMap = grunt.file.read(this.data.source + '.map');
    var inLines = inFile.split('\n');
    var i = 0; // Discover copyright header

...
axs.color.colorChannelToString = function (value) {
  ...
  if (value <= 0xF) return '0' + value.toString(16);
...
axs.color.colorToString = function (color) {
  if (color.alpha == 1) {
    return '#' + axs.color.colorChannelToString(color.red) + axs.color.colorChannelToString(color.green) + axs.color.colorChannelToString(color.blue);
  } else return 'rgba(' + [color.red, color.green, color.blue, color.alpha].join(',') + ')';
};
/**
...
axs.color.translateColor = function (ycc, luma) {
  ...
  var cubeFaces = endpoint == axs.color.WHITE_YCC ? axs.color.YCC_CUBE_FACES_WHITE : axs.color.YCC_CUBE_FACES_BLACK;
  ...
  for (var i = 0; i < cubeFaces.length; i++) {
    ...
    intersection = axs.color.findIntersection(line, cubeFace); // If intersection within [0, 1] in Z axis, it is within the cube.

    if (intersection.z >= 0 && intersection.z <= 1) break;
  }
...
axs.color.suggestColors = function (bgColor, fgColor, desiredContrastRatios) {
  ...
  for (var desiredLabel in desiredContrastRatios) {
    ...
    if (desiredFgLuminance <= 1 && desiredFgLuminance >= 0) {
      ...
      suggestedColors.fg =
      /** @type {!string} */
      axs.color.colorToString(newFgColor);
      suggestedColors.bg =
      /** @type {!string} */
      axs.color.colorToString(bgColor);
      suggestedColors.contrast =
      /** @type {!string} */
      newContrastRatio.toFixed(2);
      colors[desiredLabel] =
      /** @type {axs.color.SuggestedColors} */
      suggestedColors;
      continue;
    }
    ...
    if (desiredBgLuminance <= 1 && desiredBgLuminance >= 0) {
      ...
      suggestedColors.bg =
      /** @type {!string} */
      axs.color.colorToString(newBgColor);
      suggestedColors.fg =
      /** @type {!string} */
...
axs.color.invert3x3Matrix = function (matrix) {
  ...
  var z = 1 / det;
  return axs.color.scalarMultiplyMatrix([[A, D, G], [B, E, H], [C, F, K]], z);
};
...
axs.color.scalarMultiplyMatrix = function (matrix, scalar) {
  ...
  for (var i = 0; i < 3; i++) result[i] = axs.color.scalarMultiplyVector(matrix[i], scalar);
...
axs.color.scalarMultiplyVector = function (vector, scalar) {
  ...
  for (var i = 0; i < vector.length; i++) result[i] = vector[i] * scalar;

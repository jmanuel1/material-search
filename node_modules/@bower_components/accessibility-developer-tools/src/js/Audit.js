...
axs.AuditConfiguration.prototype = {
  ...
  ignoreSelectors: function (auditRuleName, selectors) {
    if (!(auditRuleName in this.rules_)) this.rules_[auditRuleName] = {};
    if (!('ignore' in this.rules_[auditRuleName])) this.rules_[auditRuleName].ignore = [];
  ...
  getIgnoreSelectors: function (auditRuleName) {
    if (auditRuleName in this.rules_ && 'ignore' in this.rules_[auditRuleName]) {
  ...
  getSeverity: function (auditRuleName) {
    if (!(auditRuleName in this.rules_)) return null;
    if (!('severity' in this.rules_[auditRuleName])) return null;
  ...
  getRuleConfig: function (auditRuleName) {
    if (!(auditRuleName in this.rules_)) return null;
    if (!('config' in this.rules_[auditRuleName])) return null;
...
axs.Audit.run = function (opt_configuration) {
  ...
  var auditRules = axs.AuditRules.getActiveRules(configuration); // As a performance optimization set the collectIdRefs flag to false if none of the rules needs it.
  ...
  for (var i = 0; i < auditRules.length; i++) {
    ...
    if (!auditRule.canRun(configuration)) continue;
...
(function () {
  ...
  axs.Audit.collectMatchingElements = function (configuration, auditRules) {
    ...
    var root = configuration.walkDom ? document.documentElement : configuration.scope; // Because 'related elements' could occur anywhere in the DOM we need to start at document.documentElement

    axs.dom.composedTreeSearch(root, null, {
      preorder: function (element, flags) {
        if (!flags.inScope) flags.inScope = element === configuration.scope;
        ...
        for (var i = 0; i < auditRules.length; i++) {
          ...
          var ignore = flags.ignoring[auditRule.name] || (flags.ignoring[auditRule.name] = auditRule._options.shouldIgnore(element));
  ...
  function AuditOptions(configuration, auditRule) {
    ...
    if (ignoreSelectors.length > 0 || configuration.scope) this.ignoreSelectors = ignoreSelectors;
    var ruleConfig = configuration.getRuleConfig(auditRule.name);
    if (ruleConfig) this.config = ruleConfig;
  }
  /**
  ...
  AuditOptions.prototype.shouldIgnore = function (element) {
    ...
    if (ignoreSelectors) {
      for (var i = 0; i < ignoreSelectors.length; i++) {
        if (axs.browserUtils.matchSelector(element, ignoreSelectors[i])) return true;
      }
    }
...
axs.Audit.auditResults = function (results) {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (result.result != axs.constants.AuditResult.FAIL) continue;

    if (result.rule.severity == axs.constants.Severity.SEVERE) {
      auditResults.addError(axs.Audit.accessibilityErrorMessage(result));
    } else {
      auditResults.addWarning(axs.Audit.accessibilityErrorMessage(result));
    }
  }
...
axs.Audit.accessibilityErrorMessage = function (result) {
  if (result.rule.severity == axs.constants.Severity.SEVERE) var message = 'Error: ';else var message = 'Warning: ';
  message += result.rule.code + ' (' + result.rule.heading + ') failed on the following ' + (result.elements.length == 1 ? 'element' : 'elements');
  if (result.elements.length == 1) message += ':';else {
    message += ' (1 - ' + Math.min(5, result.elements.length) + ' of ' + result.elements.length + '):';
  }

...
axs.AuditRule = function (spec) {
  ...
  this.isRelevant_ = spec.isRelevant || function (element, flags) {
...
axs.AuditRule.requiredFields = ['name', 'severity', 'relevantElementMatcher', 'test', 'code', 'heading'];
/**
...
axs.AuditRule.Result.prototype.update = function (auditResult, element) {
  ...
  if (auditResult === axs.constants.AuditResult.FAIL) {
    ...
    if (result.result !== axs.constants.AuditResult.FAIL) result.result = auditResult; // If PASS then we change result only if it is not already FAIL

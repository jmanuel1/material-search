...
axs.constants.ARIA_ROLES = {
  ...
  "roletype": {
    "abstract": true,
    "properties": ["aria-atomic", "aria-busy", "aria-controls", "aria-describedby", "aria-disabled", "aria-dropeffect", "aria-flowto", "aria-grabbed", "aria-haspopup", "aria-hidden", "aria-invalid", "aria-label", "aria-labelledby", "aria-live", "aria-owns", "aria-relevant"]
  },
  "row": {
...
axs.constants.addAllParentRolesToSet_ = function (role, set) {
  ...
  for (var j = 0; j < parents.length; j++) {
    ...
    axs.constants.addAllParentRolesToSet_(axs.constants.ARIA_ROLES[parentRoleName], set);
  }
};
/**
...
axs.constants.addAllPropertiesToSet_ = function (role, propertiesName, propertiesSet) {
  var properties = role[propertiesName];

  if (properties) {
    for (var i = 0; i < properties.length; i++) propertiesSet[properties[i]] = true;
  }
  ...
  if (role['parent']) {
    ...
    for (var j = 0; j < parents.length; j++) {
      var parentRoleName = parents[j];
      axs.constants.addAllPropertiesToSet_(axs.constants.ARIA_ROLES[parentRoleName], propertiesName, propertiesSet);
    }
  }
...
for (var roleName in axs.constants.ARIA_ROLES) {
  ...
  if ('widget' in parentRolesSet) axs.constants.WIDGET_ROLES[roleName] = role;
...
(function () {
  ...
  for (var propertyName in axs.constants.ARIA_PROPERTIES) {
    ...
    if (!propertyDetails.values) continue;
    var valuesSet = {};

    for (var i = 0; i < propertyDetails.values.length; i++) valuesSet[propertyDetails.values[i]] = true;
...
axs.constants.TAG_TO_IMPLICIT_SEMANTIC_INFO = {
  'A': [{
    role: 'link',
    allowed: ['button', 'checkbox', 'menuitem', 'menuitemcheckbox', 'menuitemradio', 'tab', 'treeitem'],
    selector: 'a[href]'
  }],
  'ADDRESS': [{
    role: '',
  ...
  'DIALOG': [{
    // updated 'allowed' from: http://www.w3.org/html/wg/drafts/html/master/interactive-elements.html#the-dialog-element
    role: 'dialog',
  ...
  'INPUT': [{
    ...
    role: 'combobox',
    // aria-owns is set to the same value as the list attribute
  ...
  'LI': [{
    role: 'listitem',
    allowed: ['menuitem', 'menuitemcheckbox', 'menuitemradio', 'option', 'tab', 'treeitem', 'presentation'],
    selector: 'ol:not([role="presentation"])>li, ul:not([role="presentation"])>li'
  }, {
    role: 'listitem',
    allowed: ['listitem', 'menuitem', 'menuitemcheckbox', 'menuitemradio', 'option', 'tab', 'treeitem', 'presentation'],
  ...
  'SECTION': [{
    role: 'region',
    allowed: ['alert', 'alertdialog', 'application', 'contentinfo', 'dialog', 'document', 'log', 'marquee', 'search', 'status', 'presentation']
  }],
  'SELECT': [{
    role: 'listbox'
  }],

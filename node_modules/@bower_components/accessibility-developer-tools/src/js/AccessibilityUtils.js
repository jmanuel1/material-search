...
axs.utils.elementIsOutsideScrollArea = function (element) {
  ...
  while (parent != defaultView.document.body) {
    if (axs.utils.isClippedBy(element, parent)) return true;
    if (axs.utils.canScrollTo(element, parent) && !axs.utils.elementIsOutsideScrollArea(parent)) return false;
...
axs.utils.canScrollTo = function (element, container) {
  ...
  if (rect.right < containerScrollArea.left || rect.bottom < containerScrollArea.top || rect.left > containerScrollArea.right || rect.top > containerScrollArea.bottom) {
    return false;
  }
  ...
  if (rect.left > containerRect.right || rect.top > containerRect.bottom) {
    return style.overflow == 'scroll' || style.overflow == 'auto' || container instanceof defaultView.HTMLBodyElement;
  }
...
axs.utils.isClippedBy = function (element, container) {
  ...
  if ((rect.right < containerRect.left || rect.bottom < containerRect.top || rect.left > containerRect.right || rect.top > containerRect.bottom) && style.overflow == 'hidden') {
    return true;
  }

  if (rect.right < containerScrollArea.left || rect.bottom < containerScrollArea.top) return style.overflow != 'visible';
  return false;
};
/**
...
axs.utils.overlappingElements = function (element) {
  if (axs.utils.elementHasZeroArea(element)) return null;
  ...
  for (var i = 0; i < clientRects.length; i++) {
    ...
    if (elementAtPoint == null || elementAtPoint == element || axs.utils.isAncestor(elementAtPoint, element) || axs.utils.isAncestor(element, elementAtPoint)) {
      continue;
    }

    var overlappingElementStyle = window.getComputedStyle(elementAtPoint, null);
    if (!overlappingElementStyle) continue;
    var overlappingElementBg = axs.utils.getBgColor(overlappingElementStyle, elementAtPoint);

    if (overlappingElementBg && overlappingElementBg.alpha > 0 && overlappingElements.indexOf(elementAtPoint) < 0) {
...
axs.utils.elementIsHtmlControl = function (element) {
  var defaultView = element.ownerDocument.defaultView; // HTML control
...
axs.utils.elementIsAriaWidget = function (element) {
  if (element.hasAttribute('role')) {
    var roleValue = element.getAttribute('role'); // TODO is this correct?
...
axs.utils.elementIsVisible = function (element) {
  if (axs.utils.elementIsTransparent(element)) return false;
  if (axs.utils.elementHasZeroArea(element)) return false;
  if (axs.utils.elementIsOutsideScrollArea(element)) return false;
  var overlappingElements = axs.utils.overlappingElements(element);
  if (overlappingElements.length) return false;
  return true;
};
/**
...
axs.utils.isLargeFont = function (style) {
  ...
  if (matches) {
    ...
    if (bold && fontSizeEm >= 1.2 || fontSizeEm >= 1.5) return true;
    return false;
  }
  ...
  if (matches) {
    ...
    if (bold && fontSizePercent >= 120 || fontSizePercent >= 150) return true;
    return false;
  }
  ...
  if (matches) {
    ...
    if (bold && fontSizePt >= 14 || fontSizePt >= 18) return true;
    return false;
  }
...
axs.utils.getBgColor = function (style, element) {
  ...
  if (style.opacity < 1) bgColor.alpha = bgColor.alpha * style.opacity;

  if (bgColor.alpha < 1) {
    var parentBg = axs.utils.getParentBgColor(element);
    if (parentBg == null) return null;
...
axs.utils.getParentBgColor = function (element) {
  ...
  while (parent = axs.dom.parentElement(parent)) {
    ...
    if (computedStyle.opacity < 1) parentBg.alpha = parentBg.alpha * computedStyle.opacity;
    if (parentBg.alpha == 0) continue;
  ...
  if (!foundSolidColor) bgStack.push(new axs.color.Color(255, 255, 255, 1));
  var bg = bgStack.pop();
...
axs.utils.getFgColor = function (style, element, bgColor) {
  ...
  if (fgColor.alpha < 1) fgColor = axs.color.flattenColors(fgColor, bgColor);
...
axs.utils.getContrastRatioForElementWithComputedStyle = function (style, element) {
  if (axs.utils.isElementHidden(element)) return null;
...
axs.utils.isLowContrast = function (contrastRatio, style, opt_strict) {
  ...
  if (!opt_strict) {
    return roundedContrastRatio < 3.0 || !axs.utils.isLargeFont(style) && roundedContrastRatio < 4.5;
  } else {
    return roundedContrastRatio < 4.5 || !axs.utils.isLargeFont(style) && roundedContrastRatio < 7.0;
  }
};
/**
 * @param {Element} element
...
axs.utils.hasLabel = function (element) {
  ...
  if (tagName == 'img' && element.hasAttribute('alt')) return true;
  if (tagName == 'input' && type == 'image' && element.hasAttribute('alt')) return true;
  if (tagName == 'input' && (type == 'submit' || type == 'reset')) return true; // There's a separate audit that makes sure this points to an actual element or elements.

  if (element.hasAttribute('aria-labelledby')) return true;

  if (element.hasAttribute('id')) {
    var labelsFor = document.querySelectorAll('label[for="' + element.id + '"]');
    if (labelsFor.length > 0) return true;
  }
  ...
  while (parent) {
    if (parent.tagName.toLowerCase() == 'label') {
      ...
      /** HTMLLabelElement */
      parent;
      if (parentLabel.control == element) return true;
    }
...
axs.utils.isElementDisabled = function (element, ignoreAncestors) {
  ...
  if (!axs.utils.isNativelyDisableable(element) || axs.browserUtils.matchSelector(element, 'fieldset>legend:first-of-type *')) {
    return false;
  }
...
axs.utils.isElementOrAncestorHidden = function (element) {
  if (axs.utils.isElementHidden(element)) return true;
  if (axs.dom.parentElement(element)) return axs.utils.isElementOrAncestorHidden(axs.dom.parentElement(element));else return false;
};
/**
 * @param {Element} element An element to check
...
axs.utils.getRoles = function (element, implicit) {
  if (!element || element.nodeType !== Node.ELEMENT_NODE || !element.hasAttribute('role') && !implicit) return null;
  var roleValue = element.getAttribute('role');
  if (!roleValue && implicit) roleValue = axs.properties.getImplicitRole(element);
  ...
  switch (propertyType) {
    ...
    case "idref_list":
      ...
      for (var i = 0; i < idrefValues.length; i++) {
        ...
        if (!refIsValid.valid) result.valid = false;
        if (result.values) result.values.push(refIsValid);else result.values = [refIsValid];
      }
    ...
    case "token_list":
      ...
      for (var i = 0; i < tokenValues.length; i++) {
        ...
        if (result.values) result.values.push(validTokenValue.value);else result.values = [validTokenValue.value];
      }
...
axs.utils.isPossibleValue = function (value, possibleValues, propertyName) {
  if (!possibleValues[value]) return {
    'valid': false,
    'value': value,
...
axs.utils.isValidBoolean = function (value) {
  ...
  if (typeof parsedValue != 'boolean') return {
    'valid': false,
    'value': value,
...
axs.utils.isValidIDRefValue = function (value, element) {
  ...
  if (!element.ownerDocument.getElementById(value)) return {
    'valid': false,
    'idref': value,
...
axs.utils.isElementImplicitlyFocusable = function (element) {
  var defaultView = element.ownerDocument.defaultView;
  if (element instanceof defaultView.HTMLAnchorElement || element instanceof defaultView.HTMLAreaElement) return element.hasAttribute('href');
  if (element instanceof defaultView.HTMLInputElement || element instanceof defaultView.HTMLSelectElement || element instanceof defaultView.HTMLTextAreaElement || element instanceof defaultView.HTMLButtonElement || element instanceof defaultView.HTMLIFrameElement) return !element.disabled;
  return false;
};
/**
...
axs.utils.values = function (obj) {
  ...
  for (var key in obj) {
    if (obj.hasOwnProperty(key) && typeof obj[key] != 'function') values.push(obj[key]);
  }
...
axs.utils.namedValues = function (obj) {
  ...
  for (var key in obj) {
    if (obj.hasOwnProperty(key) && typeof obj[key] != 'function') values[key] = obj[key];
  }
...
function escapeId(id) {
  return id.replace(/[^a-zA-Z0-9_-]/g, function (match) {
    return '\\' + match;
  });
}
...
axs.utils.getQuerySelectorText = function (obj) {
  ...
  if (obj.hasAttribute) {
    ...
    if (obj.className) {
      ...
      for (var i = 0; i < obj.classList.length; i++) selector += '.' + obj.classList[i];
      ...
      if (obj.parentNode) {
        for (i = 0; i < obj.parentNode.children.length; i++) {
          ...
          if (axs.browserUtils.matchSelector(similar, selector)) total++;
          if (similar === obj) break;
        }
      } else {
        total = 1;
      }
      ...
      if (total == 1) {
        return axs.utils.getQuerySelectorText(obj.parentNode) + ' > ' + selector;
      }
    }
    ...
    if (obj.parentNode) {
      ...
      if (total == 1) {
        return next + obj.tagName;
      } else {
        return next + obj.tagName + ':nth-of-type(' + total + ')';
      }
    }
...
axs.utils.getHtmlIdReferrers = function (element) {
  ...
  var selectorTemplates = ['[contextmenu=\'{id}\']', '[itemref~=\'{id}\']', 'button[form=\'{id}\']', 'button[menu=\'{id}\']', 'fieldset[form=\'{id}\']', 'input[form=\'{id}\']', 'input[list=\'{id}\']', 'keygen[form=\'{id}\']', 'label[for=\'{id}\']', 'label[form=\'{id}\']', 'menuitem[command=\'{id}\']', 'object[form=\'{id}\']', 'output[for~=\'{id}\']', 'output[form=\'{id}\']', 'select[form=\'{id}\']', 'td[headers~=\'{id}\']', 'textarea[form=\'{id}\']', 'tr[headers~=\'{id}\']'];
  ...
  for (var i = 0; i < element.attributes.length; i++) {
    ...
    if (attrib.specified) {
      ...
      switch (attribName) {
        ...
        case 'form':
          if (tagName == 'button' || tagName == 'fieldset' || tagName == 'input' || tagName == 'keygen' || tagName == 'label' || tagName == 'object' || tagName == 'output' || tagName == 'select' || tagName == 'textarea') {
            addResult(attrib.value);
          }
...
axs.utils.getIdReferents = function (attributeName, element) {
  ...
  var property = axs.constants.ARIA_PROPERTIES[propertyKey];
  if (!property || !element.hasAttribute(attributeName)) return result;
  var propertyType = property.valueType;
...
axs.utils.findDescendantsWithRole = function (element, role) {
  ...
  if (result) {
    // Convert NodeList to Array; methinks 80/20 that's what callers want.
    result = Array.prototype.map.call(result, function (item) {
      return item;
    });
  } else {
    return [];

...
(function () {
  ...
  axs.AuditRules.addRule = function (spec) {
    ...
    if (auditRule.code in auditRulesByCode) throw new Error('Can not add audit rule with same code: "' + auditRule.code + '"');
    if (auditRule.name in auditRulesByName) throw new Error('Can not add audit rule with same name: "' + auditRule.name + '"');
  ...
  axs.AuditRules.getRules = function (opt_namesOnly) {
    ...
    if (opt_namesOnly) return ruleNames;
  ...
  axs.AuditRules.getActiveRules = function (configuration) {
    ...
    if (configuration.auditRulesToIgnore) {
      for (var i = 0; i < configuration.auditRulesToIgnore.length; i++) {
        ...
        if (auditRules.indexOf(auditRuleToIgnore) < 0) continue;

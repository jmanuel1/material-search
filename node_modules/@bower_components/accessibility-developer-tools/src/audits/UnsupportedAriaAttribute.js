...
(function () {
  'use strict'; // Over many iterations it makes a significant performance difference not to re-instantiate regex

  var ARIA_ATTR_RE = /^aria\-/; // No need to compute the selector for every element in the DOM.
  ...
  var unsupportedAriaAttribute = {
    ...
    test: function (element) {
      ...
      if (role && role.applied) {
        ...
        role.applied.details.propertiesSet;
      } else {
      ...
      for (var i = 0, len = attributes.length; i < len; i++) {
        ...
        if (ARIA_ATTR_RE.test(attributeName)) {
          var lookupName = attributeName.replace(ARIA_ATTR_RE, ''); // we're only interested in known aria properties

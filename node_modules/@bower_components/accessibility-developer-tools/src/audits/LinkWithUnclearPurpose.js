...
axs.AuditRules.addRule({
  ...
  test: function (anchor, flags, opt_config) {
    ...
    for (var i = 0; i < blacklistPhrases.length; i++) {
      ...
      var phraseREString = '^\\s*' + blacklistPhrases[i].trim().replace(whitespaceRE, '\\s*') + '\s*[^a-z]$';
      var phraseRE = new RegExp(phraseREString, 'i');
      if (phraseRE.test(anchor.textContent)) return true;
    } // Remove punctuation from phrase, then strip out all stopwords. Fail if remaining text is
    // all whitespace.


    var stopwords = config['stopwords'] || ['click', 'tap', 'go', 'here', 'learn', 'more', 'this', 'page', 'link', 'about'];
    var filteredText = axs.properties.findTextAlternatives(anchor, {});
    if (filteredText === null || filteredText.trim() === '') return true;
    ...
    for (var i = 0; i < stopwords.length; i++) {
      ...
      if (filteredText.trim() == '') return true;
    }

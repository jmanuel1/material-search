...
(function () {
  ...
  var spec = {
    ...
    relevantElementMatcher: function (element) {
      if (!axs.browserUtils.matchSelector(element, '[role]')) return false;
    ...
    test: function (element) {
      ...
      for (var i = required.length - 1; i >= 0; i--) {
        ...
        if (descendants && descendants.length) {
          // if we found at least one descendant with a required role
          return false;
        }
      ...
      for (var i = ownedElements.length - 1; i >= 0; i--) {
        ...
        if (ownedElementRole && ownedElementRole.applied) {
          ...
          for (var j = required.length - 1; j >= 0; j--) {
            if (appliedRole.name === required[j]) {
              // if this explicitly owned element has a required role
              return false;
            }
          }
        }
  ...
  function getRequired(element) {
    ...
    if (!elementRole || !elementRole.applied) return [];
    var appliedRole = elementRole.applied;
    if (!appliedRole.valid) return [];

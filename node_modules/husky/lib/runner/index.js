...
function run([, scriptPath, hookName = '', HUSKY_GIT_PARAMS], getStdinFn = get_stdin_1.default) {
  return __awaiter(this, void 0, void 0, function* () {
    // Update CWD
    const cwd = path_1.default.resolve(scriptPath.split('node_modules')[0]); // Debug

    debug_1.default(`cwd: ${cwd} (updated)`); // In some cases, package.json may not exist
    // For example, when switching to gh-page branch

    let pkg;

    try {
      pkg = read_pkg_1.default.sync({
        cwd,
        normalize: false
      });
    } catch (err) {
    ...
    const oldCommand = pkg && pkg.scripts && pkg.scripts[hookName.replace('-', '')]; // Run command

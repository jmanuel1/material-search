...
const hookList = ['applypatch-msg', 'pre-applypatch', 'post-applypatch', 'pre-commit', 'prepare-commit-msg', 'commit-msg', 'post-commit', 'pre-rebase', 'post-checkout', 'post-merge', 'pre-push', 'pre-receive', 'update', 'post-receive', 'post-update', 'push-to-checkout', 'pre-auto-gc', 'post-rewrite', 'sendemail-validate'];
...
function createHook(filename, script) {
  ...
  const name = path_1.default.basename(filename); // Check if hook exist

  if (fs_1.default.existsSync(filename)) {
    const hook = fs_1.default.readFileSync(filename, 'utf-8'); // Migrate
...
function install(topLevel, gitDir, huskyDir, isCI, requireRunNodePath = require.resolve('run-node/run-node')) {
  ...
  const conf = getConf_1.default(userPkgDir); // Checks
